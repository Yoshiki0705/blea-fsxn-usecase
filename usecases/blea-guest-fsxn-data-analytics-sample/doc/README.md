...
1. Users write files to FSx for ONTAP via NFS/SMB
...
| `fsxnStorageCapacityGiB` | FSx for ONTAP storage capacity (GiB) | 1024 | 2048+ |
| `fsxnThroughputCapacityMBps` | FSx for ONTAP throughput (MBps) | 128 | 512+ |
...
- **Production**: Use a dedicated service account (e.g., `analytics-svc`) registered in the FSx for ONTAP SVM's name service with appropriate file permissions
...
After deploying and placing data on the FSx for ONTAP volume, run the Glue Crawler manually:
...
> ⚠️ FSx for ONTAP file system, volumes, and KMS key have `RemovalPolicy.RETAIN` and will persist after stack deletion. Delete manually via AWS Console or CLI if needed.

## Cost Estimate

| Configuration | Monthly Cost (USD) | Breakdown |
|---------------|-------------------|-----------|
| Dev (SINGLE_AZ, 128MBps, 1TiB) | ~$500 | FSx for ONTAP SSD $200 + throughput $180 + capacity pool $15 + VPC Endpoints $50 + Glue/Athena usage |
| Prod (MULTI_AZ, 512MBps, 2TiB) | ~$1,500 | FSx for ONTAP SSD $400 + throughput $720 + capacity pool $30 + VPC Endpoints $50 + Glue/Athena usage |
...
- FSx for ONTAP security group allows VPC-internal traffic only
...
| Encryption at Rest | KMS CMK with auto-rotation | FSx for ONTAP, S3, Athena results |
...
| Resource Protection | RemovalPolicy.RETAIN + Termination Protection | FSx for ONTAP, Volumes, KMS, S3 |
