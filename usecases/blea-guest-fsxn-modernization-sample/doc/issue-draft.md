...
Without a unified storage pattern, organizations create data silos per compute platform. **This use case provides a single FSx for ONTAP storage layer accessible by all patterns** via two access methods: NFS mount (EC2/Batch) and S3 Access Point (Lambda/ECS).
...
- **Single stack design**: All patterns share the same VPC and FSx for ONTAP. Resource count (61 max) stays within CloudFormation limits. Split into multiple stacks would require cross-stack references for FSx for ONTAP, adding complexity.
...
|---------------|---------------|
| Minimum (FSx for ONTAP + EC2 + Lambda) | ~$550 |
