# BLEA ゲストシステム: FSx for ONTAP モダナイゼーションプラットフォーム
...
> **ECS Fargate の注意**: Fargate は FSx for ONTAP NFS を直接マウント**できません**。S3 AP 経由でデータにアクセスします。POSIX ファイル操作が必要な場合は EC2 パターンを使用してください。
...
| 構成 | 月額 (USD) |
|------|-----------|
| 最小 (FSx for ONTAP + EC2 + Lambda) | ~$550 |
| フル (全パターン ON) | ~$2,000 |
| VPC Endpoint (8個 × 2AZ) | ~$115 |

## 期待される効果

| 指標 | Before (VMware) | After (AWS + FSx for ONTAP) |
|------|-----------------|-------------------|
